...
import { Shield, KeyRound, RefreshCw, AlertOctagon, Trash2, ArrowLeft, CheckCircle, Copy, Clock, ShieldAlert } from 'lucide-react';
...
export const AgentDetailPage = () => {
  ...
  // Modal State
  const [modalType, setModalType] = useState(null); // 'generate' | 'rotate' | 'renew' | 'revoke' | 'delete'
  ...
  const fetchAgentDetails = async () => {
    ...
    try {
      ...
      // Fetch governance score breakdown
      try {
      ...
      // Fetch AI summary
      try {
        const sumRes = await api.post('/governance/identity-summary', {
          agent_name: res.data.agent_name,
          purpose: res.data.purpose,
          department: res.data.department,
          scopes: res.data.allowed_scopes,
  ...
  const handleGenerateCredential = async () => {
    ...
    try {
      ...
      setGenSecretResult(res.data.raw_credential);
      fetchAgentDetails();
    } catch (err) {
  ...
  const handleRenewCredential = async () => {
    ...
    setActionError('');
    try {
      await api.post('/credentials/renew', {
        agent_id: id,
        extend_days: Number(expiresInDays)
      });
      setModalType(null);
      fetchAgentDetails();
    } catch (err) {
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', fontSize: '0.9rem' }}>
            ...
            {summary && (
              <div style={{ background: 'rgba(15, 23, 42, 0.6)', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-md)', padding: '1rem', marginTop: '0.5rem' }}>
                <span style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--accent-cyan)', textTransform: 'uppercase' }}>AI Identity Summary</span>
                <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginTop: '0.35rem' }}>{summary}</p>
              </div>
            )}
          </div>
        </div>
              ...
              <div>
                {(modalType === 'generate' || modalType === 'rotate' || modalType === 'renew') && (
                  <div className="form-group">
                    <label className="form-label">Expiration Period (Days)</label>
