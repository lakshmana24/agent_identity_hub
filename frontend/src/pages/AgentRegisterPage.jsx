...
export const AgentRegisterPage = () => {
  ...
  const fetchAvailableScopes = async () => {
    try {
      ...
      setAllScopes(res.data);
      // Pre-select default read scopes
  ...
  const handleAnalyzePurpose = async () => {
    ...
    try {
      const res = await api.post('/governance/scope-recommendation', { purpose });
  ...
  return (
    <div style={{ maxWidth: '800px' }}>
      ...
      <div style={{ marginBottom: '1.75rem' }}>
        ...
        <p style={{ color: 'var(--text-muted)' }}>Provision a managed identity with scoped credentials and AI governance analysis</p>
      </div>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
            ...
            <div className="form-group">
              ...
              <input type="text" className="input-field" placeholder="e.g. Finance, Support, Engineering" value={department} onChange={(e) => setDepartment(e.target.value)} required />
            </div>
          </div>
          ...
          <div className="form-group">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <label className="form-label">Business Purpose & Tasks</label>
              <button type="button" className="btn btn-secondary" style={{ padding: '0.3rem 0.7rem', fontSize: '0.8rem' }} onClick={handleAnalyzePurpose} disabled={aiLoading || !purpose.trim()}>
                ...
                {aiLoading ? 'Analyzing...' : 'AI Scope Recommendation'}
              </button>
            </div>
            <textarea className="textarea-field" rows={3} placeholder="Describe what tasks this AI agent will perform (e.g. Handles customer support refund queries via Stripe and updates tickets)..." value={purpose} onChange={(e) => setPurpose(e.target.value)} required />
          </div>

          {/* AI Recommendation Banner */}
          {aiRecommendation && (
            <div style={{ background: 'rgba(139, 92, 246, 0.1)', border: '1px solid rgba(139, 92, 246, 0.3)', borderRadius: 'var(--radius-md)', padding: '1rem', marginBottom: '1.5rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                <span style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--accent-purple)', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                  <Sparkles size={16} /> AI Governance Assessment
                </span>
          ...
          <div className="form-group" style={{ marginBottom: '2rem' }}>
            <label className="form-label">API Scope Permissions (Principle of Least Privilege)</label>
